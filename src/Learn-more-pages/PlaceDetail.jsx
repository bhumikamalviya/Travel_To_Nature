...
export function PlaceDetail() {
  ...
  // ✅ Type ke hisaab se data choose karo
  let data = [];
  if (type === "mountains") data = mountains;
  if (type === "beaches") data = beaches;
  if (type === "forests") data = forests;

  // ✅ Find by id
  const place = data.find((p) => p.id === parseInt(id));

  // ✅ Default main image
  const [mainImage, setMainImage] = useState(place ? place.image : "");
        ...
        <div className="p-4 flex flex-col justify-center">
          ...
          {place.height_m && (
            <p className="mt-2 font-medium text-gray-600 font-['Roboto']">
              Height: {place.height_m} m
            </p>
          )}

          {/* Activities (agar ho to) */}
          {place.activitiess && (
            <div className="mt-6">

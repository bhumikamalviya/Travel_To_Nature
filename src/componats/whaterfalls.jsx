...
import waterfalls from "../../src/data/waterfalls.json"; 
...
export function Waterfalls() {
  ...
  // ⭐ Like Handler
  const handleLike = (waterfall) => {
    if (likedItems.some((i) => i.id === waterfall.id)) {
      showNotification(`Removed ${waterfall.name} from wishlist`, "remove");
    } else {
      showNotification(`Added ${waterfall.name} to wishlist ❤`, "add");
    }
    toggleLike(waterfall);
  };
      ...
      <div className="px-6 py-4 grid grid-cols-1 bg-[#e6e6e6] sm:grid-cols-2 lg:grid-cols-3 gap-4 justify-items-center">
        {waterfalls.map((waterfall) => (
          ...
          >
            ...
            <div className="relative group">
              ...
              <div className="absolute top-2 right-2 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10">
                ...
                <button
                  ...
                  onClick={() => handleLike(waterfall)}
                >
                  {likedItems.some((i) => i.id === waterfall.id) ? (
                ...
                {/* <button
                  className="bg-white py-1 px-2 rounded-lg shadow"
                  onClick={() => handleWishlist(waterfall)}
                >
                  <FaPlus
                    size={20}
                    className={
                      wishlistItems.some((i) => i.id === waterfall.id)
                        ? "text-green-600"
                        : ""
                    }
                  />
            ...
            {/* Details Section */}
            <div className="p-4">
              <h2 className="text-xl font-bold">{waterfall.name}</h2>
              <p className="text-green-600">{waterfall.location}</p>
              <p className="mt-2 text-sm text-gray-700 italic">
                {waterfall.description}
              </p>
              <p className="mt-2 text-sm font-medium text-green-600">
                Best Time: {waterfall.best_time_to_visit}
              </p>
              ...
              <button
                ...
                onClick={() => alert(`Learn more about ${waterfall.name}`)}
              >
                Learn More
              </button>
            </div>

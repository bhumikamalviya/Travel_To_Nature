...
import parks from "../../src/data/nationalParks.json";
...
export function Parks() {
  ...
  // 🔔 Notification function (same as Waterfalls)
  ...
  // ⭐ Like Handler
  const handleLike = (park) => {
    if (likedItems.some((i) => i.id === park.id)) {
      showNotification(`Removed ${park.name} from Wishlist`, "remove");
    } else {
      showNotification(`Added ${park.name} to Wishlist ❤`, "add");
    }
    toggleLike(park);
  };
        ...
        {parks.map((park) => (
          ...
          >
            ...
            <div className="relative group">
              ...
              <div className="absolute top-2 right-2 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10">
                ...
                <button
                  ...
                  onClick={() => handleLike(park)}
                >
                  {likedItems.some((i) => i.id === park.id) ? (
            ...
            <div className="p-4">
              ...
              <p className="text-green-600">{park.location}</p>
              <p className="mt-2 text-sm text-gray-700 italic">
                {park.description}
              </p>
              <p className="mt-2 text-sm font-medium text-green-600">
                Best Time: {park.best_time_to_visit}
              </p>
              ...
              <button
                ...
                onClick={() => alert(`Learn more about ${park.name}`)}
              >
                Learn More
              </button>
            </div>

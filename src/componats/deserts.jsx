// src/components/Deserts.jsx
import React, { useState } from "react";
import { FaRegHeart, FaHeart } from "react-icons/fa";
import { FaPlus } from "react-icons/fa6";
import { Destinations } from "../pages/destinations";
import deserts from "../../src/data/deserts.json";
...
export function Deserts() {
  ...
  // 🔔 Notification function (same style as Forests.jsx)
  ...
  // ⭐ Wishlist handler
  const handleWishlist = (desert) => {
    if (wishlistItems.some((i) => i.id === desert.id)) {
      showNotification(`Removed ${desert.name} from wishlist`, "remove");
    } else {
      showNotification(`Added ${desert.name} to wishlist ❤`, "add");
    }
    toggleWishlist(desert);
  };

  // ⭐ Like handler
  const handleLike = (desert) => {
    if (likedItems.some((i) => i.id === desert.id)) {
      showNotification(`Removed ${desert.name} from wishlist`, "remove");
    } else {
      showNotification(`Added ${desert.name} to wishlist❤`, "add");
    }
        ...
        {notifications.map((n) => (
          <div
            ...
            className={`px-3 py-2 rounded-md shadow-lg w-56 text-sm
              ...
              ${n.type === "default" ? "bg-green-100 text-green-700" : ""}
            `}
          >
            {n.message}
        ...
        {deserts.map((desert) => (
          ...
          >
            ...
            <div className="relative group">
              ...
              <div className="absolute top-2 right-2 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10">
                ...
                <button
                  ...
                  onClick={() => handleLike(desert)}
                >
                  {likedItems.some((i) => i.id === desert.id) ? (
            ...
            <div className="p-4">
              ...
              <p className="text-green-600">{desert.location}</p>
              <p className="mt-2 text-sm text-gray-700 italic">
                {desert.description}
              </p>
              <p className="mt-2 text-sm font-medium text-green-600">
                Best Time: {desert.best_time_to_visit}
              </p>
              <p className="mt-1 text-sm text-gray-600">
                Area: {desert.area_sq_km.toLocaleString()} km²
              </p>
              <button
                ...
                onClick={() => alert(`Learn more about ${desert.name}`)}
              >
                Learn More
              </button>
            </div>

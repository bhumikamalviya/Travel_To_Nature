// src/components/Beaches.jsx
import React, { useState } from "react";
import { FaRegHeart, FaHeart } from "react-icons/fa";
import { FaPlus } from "react-icons/fa6"; // ✅ extra button (wishlist) ke liye
...
export function Beaches() {
  ...
  // ⭐ Like handler
  const handleLike = (beach) => {
    if (likedItems.some((i) => i.id === beach.id)) {
      showNotification(`Removed ${beach.name} from wishlist`, "remove");
    } else {
      showNotification(`Added ${beach.name} to wishlist ❤`, "add");
    }
    toggleLike(beach);
  };
        ...
        {beaches.map((beach) => (
          ...
          >
            ...
            <div className="relative group">
              ...
              <div className="absolute top-2 right-2 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10">
                ...
                <button
                  ...
                  onClick={() => handleLike(beach)}
                >
                  {likedItems.some((i) => i.id === beach.id) ? (
            ...
            <div className="p-4">
              ...
              <p className="text-green-600">{beach.location}</p>
              <p className="mt-2 text-sm text-gray-700 italic">
                {beach.description}
              </p>
              <p className="mt-2 text-sm font-medium text-green-600">
                Best Time: {beach.best_time_to_visit}
              </p>
              <button
